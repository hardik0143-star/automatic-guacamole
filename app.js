...
/* ==========================================================
   Tiny Tiffin v1.1 Professional Edition — compatibility layer
   Safe additive enhancement: preserves the existing app logic.
   ========================================================== */
(function () {
  "use strict";
  ...
  function removeLegacyAiAssistant(root) {
    if (!root) return;
    root.querySelectorAll(".ai-hub, .ai-assistant, [data-feature='ai-assistant']").forEach(function (el) {
      el.remove();
    });
    root.querySelectorAll("button, a, h2, h3, p, div").forEach(function (el) {
      if (el.children.length) return;
      var text = normalise(el.textContent);
      if (text === "tiny tiffin ai" || text === "tiny tiffin ai assistant" || text === "ai assistant") {
        el.remove();
      }
    });
  }

  function enrichRecipeSearch() {
    var recipes = window.TINY_TIFFIN_RECIPES || [];
    recipes.forEach(function (recipe) {
      var source = []
        .concat(recipe.ingredients || [])
        .concat(recipe.name && Object.values(recipe.name) || [])
        .concat(recipe.desc && Object.values(recipe.desc) || [])
        .join(" ");
      ...
      Object.keys(rules).forEach(function (key) { if (rules[key].test(text)) categories.add(key); });
      recipe.ingredientCategories = Array.from(categories);
      recipe.searchText = text;
      if (/paneer/.test(text) && !/tofu/.test(text)) {
        recipe.tofuTip = recipe.tofuTip || {
          en: "For a plant-based option, use firm tofu in place of paneer and adjust seasoning to taste."
        };
      }
    });
  }
  ...
  function installSupport() {
    ...
    window.addEventListener("beforeinstallprompt", function (event) {
      ...
      document.dispatchEvent(new CustomEvent("tiny-tiffin-install-ready"));
    });
    document.addEventListener("click", async function (event) {
      var button = event.target.closest("[data-install-app], .install-action");
  ...
  function updateSupport() {
    document.addEventListener("click", function (event) {
      var button = event.target.closest("[data-update-app]");
      if (!button) return;
      event.preventDefault();
  ...
  function apply() {
    enrichRecipeSearch();
    removeLegacyAiAssistant(document.getElementById("app"));
  }
  ...
  window.TinyTiffinProfessional = window.TinyTiffinProfessional || {};
  window.TinyTiffinProfessional.version = "v1.1";
  window.TinyTiffinProfessional.refreshRecipeMetadata = enrichRecipeSearch;
})();
